...
function Emoji() {
    const [image_emoji,setImage_emoji] = React.useState('default.jpg');
    ...
    async function sendImage_emoji(e) { 
        ...
        if(!is_auth){
            ...
            chrome.runtime.sendMessage({ action: "need_auth" });
            return;
        }
        if (image_emoji == 'default.jpg') {
        ...
        reader.onloadend = async function() {
            ...
            chrome.runtime.sendMessage({ action: "send_emoji_to_background", result: body });
            insertemoji(body);
